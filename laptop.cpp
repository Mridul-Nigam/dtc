...
int main()
{
    ...
    std::thread([&encoders, &scale, &done, &engaged, &master, &mx0, &my0, &mz0, &sx0, &sy0, &sz0, &slave, &tx, &ty, &tz, &fx, &fy, &fz, &time1]() {
        ...
        while (!done && startDataTfr == 1) {
            ...
            auto str_time = std::chrono::high_resolution_clock::now();
            ...
            auto en_time = std::chrono::high_resolution_clock::now();
            auto dur1 = std::chrono::duration_cast<std::chrono::microseconds>((en_time - str_time));
            clk2 = clk2 + dur1.count();
    ...
    channel1->onOpen([&channel1]() {
        std::thread([&channel1]() {
            ...
            while (startDataTfr == 1) {
                ...
                auto dur1 = std::chrono::duration_cast<std::chrono::microseconds>((end_time - start_time));
                clk1 = clk1 + dur1.count();
            }
            ...
            //std::cout << "The array no. " << i << " sending is: "<<"GMT : "<<time4<<std::endl;
            std::cout << "generation time: " << clk1/k << "  sending time : " << clk2/k << std::endl;
