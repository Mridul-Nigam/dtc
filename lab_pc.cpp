...
int main()
{
    ...
    //    } }).detach();
        ...
        channel1->onOpen([&channel1, &encoders, &scale, &done, &engaged, &master, &mx0, &my0, &mz0, &sx0, &sy0, &sz0, &slave, &tx, &ty, &tz, &fx, &fy, &fz, &time1]() {
            std::thread([&channel1, &encoders, &scale, &done, &engaged, &master, &mx0, &my0, &mz0, &sx0, &sy0, &sz0, &slave, &tx, &ty, &tz, &fx, &fy, &fz, &time1]() {
                while (startDataTfr == 0)
                {
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
                }
                ...
                while (!done && startDataTfr == 1) {
                    ...
                    if (5 > 3) {
                        ...
                        /*std::time_t cur = std::chrono::system_clock::to_time_t(time4);
                        std::cout << "time 4 " << std::ctime(&cur) << std::endl;*/
                        //std::cout <<"GMT "<< time4 << std::endl; ----------------------------------------
                        if (i == 1) { std::cout << "start_time" << time4 << std::endl; }
                        auto dur = std::chrono::duration_cast<std::chrono::microseconds>((time3-time2));
                        clk = (clk+dur.count());
                        //ss << time3;
                        //std::cout <<dur <<" , "<< clk << std::endl; ------------------
                ...
                std::cout << "The array no. " << i << " is sent @ : " << (clk / k) << " end time: " << end_time<<std::endl;;
